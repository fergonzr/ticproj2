"""Test module for MarkEmergencyAsResolvedCommand use case.

This module contains tests for the MarkEmergencyAsResolvedCommand and its handler.
"""
...
from core.application.use_cases.mark_emergency_as_resolved import (
    ...
    MarkEmergencyAsResolvedHandler,
)
...
# Create an emergency in ON_SITE status (valid for resolution)
...
# Create an emergency in wrong status (not ON_SITE or IN_TRANSFER)
...
def mock_coordinator():
    ...
    mock.report_resolution = AsyncMock()  # Add the new method
    return mock
...
def handler(mock_coordinator, mock_storage):
    ...
    return MarkEmergencyAsResolvedHandler(
...
@pytest.mark.asyncio
async def test_successful_resolution_from_on_site(
    handler, mock_storage, mock_coordinator
):
    """Test successful resolution of emergency from ON_SITE status."""
    ...
    # Verify emergency was saved with SOLVED status
    mock_storage.save_emergency.assert_awaited_once()
    saved_emergency = mock_storage.save_emergency.call_args[0][0]
    assert saved_emergency.status.name == "SOLVED"

    # Verify resolution was reported to coordinator
    mock_coordinator.report_resolution.assert_awaited_once_with(saved_emergency)


@pytest.mark.asyncio
async def test_successful_resolution_from_in_transfer(
    handler, mock_storage, mock_coordinator
):
    """Test successful resolution of emergency from IN_TRANSFER status."""
    mock_storage.get_emergency.return_value = TEST_EMERGENCY_IN_TRANSFER
    command = MarkEmergencyAsResolvedCommand(emergencyId=TEST_EMERGENCY_ID)

...
import cqrs
from cqrs import RequestMediator, StreamingRequestMediator
...
def _generate_streaming_command_mapper(
    useCases: list[type[StreamingDefaultedRequest]],
) -> Callable[[cqrs.RequestMap], None]:
    """Generates a streaming command mapper function for the given use
    cases.
    ...
    Args:
        useCases: A list of StreamingDefaultedRequest for each use case.
...
def create_streaming_mediator(
    serviceDiscovery: ServiceDiscoveryPort,
    useCases: list[type[StreamingDefaultedRequest]],
    adapter: Port | None = None,
    **kwargs,
) -> StreamingRequestMediator:
    """Creates a streaming CQRS mediator with the given configuration.

    This function sets up a dependency injection container, registers all ports
    and use cases, and creates a configured mediator for handling
    streaming CQRS commands. It is the responsibility of the caller to
    ensure that all of the adapters required by all of the use cases
    requested are registered.

    Args:
        serviceDiscovery: The service discovery port used to retrieve
                 required services.
        useCases: A list of StreamingDefaultedRequets that the service
                 expects to be handled properly.
        adapter: An adapter that the caller already implements and
                 thus there is no need to look it up through
                 serviceDiscovery.

    Returns:
        A configured StreamingRequestMediator instance ready to handle commands.
    """
    container = containers.DynamicContainer()
    ...
    return bootstrap.bootstrap_streaming(
        di_container=cqrs_container,
        commands_mapper=_generate_streaming_command_mapper(useCases),
        message_broker=broker,
        **kwargs,
    )
